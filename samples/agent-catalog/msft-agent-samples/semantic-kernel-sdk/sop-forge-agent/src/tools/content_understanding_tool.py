...
class ContentUnderstandingTool:
    ...
    def create_analyzer(self):
        analyzer_id = "sop-forge-" + str(uuid.uuid4())

        # Create analyzer
        response = self.client.begin_create_analyzer(analyzer_id, analyzer_template_path=self.AZURE_AI_CU_SCHEMA_FILE_PATH)
    ...
    def analyze(self, intput_file_path: str, analyzer_id: str, output_directory: str) -> str:
        Logger.log_start("ContentUnderstandingTool for: " + intput_file_path)

        # Analyzer file
        response = self.client.begin_analyze(analyzer_id, file_location=intput_file_path)
        result = self.client.poll_result(response=response, timeout_seconds=600)
        ...
        for i, item in enumerate(result['result']['contents']):
            ...
            steps[step_name]['description'] = self.get_analyzer_output_field(item, 'description')
            steps[step_name]['category'] = self.get_analyzer_output_field(item, 'category')
            steps[step_name]['safetyInstruction'] = self.get_analyzer_output_field(item, 'safetyInstruction')
            steps[step_name]['keyTips'] = self.get_analyzer_output_field(item, 'keyTips')

            keyframe = "keyFrame." + self.get_analyzer_output_field(item, 'keyFrame')
            try:
                ...
                keyframe = re.findall(r"(keyFrame\.\d+)\.jpg", item['markdown']).pop(0)
                self.save_image(keyframe, response, output_directory)
            
            steps[step_name]['imageLink'] = keyframe + ".jpg"

        Logger.log_completion("ContentUnderstandingTool for: " + intput_file_path)
        return steps
