...
class SOPForgeAgent:
    ...
    async def execute(self, video_steps: list[str]) -> str:
        ...
        if not AZURE_AI_AGENT_AGENT_ID:
            with open(AGENT_INSTRUCTIONS_PATH, 'r', encoding="utf-8") as file:
                instructions = file.read()
                # Create a new agent if no agent ID is provided
                agent_definition = await client.agents.create_agent(
                    name="SOPForgeAgentSample",
                    description="SOP Forge Agent to produce a Standard Operating Procedure from a video file.",
        ...
        try:
            analysis_text = await agent.get_response(
                messages=json.dumps(video_steps),
                thread=thread
            )
