...
class ContentUnderstandingTool:
    ...
    def create_analyzer(self):
        analyzer_id = "contract-analysis-" + str(uuid.uuid4())
    ...
    def analyze(self, filepath: str, analyzer_id: str) -> str:
        Logger.log_start("ContentUnderstandingTool for: " + filepath)

        # Analyzer file
        response = self.client.begin_analyze(analyzer_id, file_location=filepath)
        result = self.client.poll_result(response)
        contract_in_markdown = result['result']['contents'][0]['markdown']
        
        Logger.log_completion("ContentUnderstandingTool for: " + filepath)
        return contract_in_markdown
